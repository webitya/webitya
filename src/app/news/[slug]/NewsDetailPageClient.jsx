...
export default function NewsDetailPageClient({ params }) {
  ...
  useEffect(() => {
    ...
    let foundArticle = newsData.find((item) => item.slug === slug)
    let sponsored = false

    if (!foundArticle) {
      foundArticle = sponsoredData.find((item) => item.slug === slug)
      sponsored = true
    }
  ...
  // SEO metadata
  const metaTitle = `${article.title} | Webitya News`
  const metaDescription = article.summary
  const metaKeywords = `${article.category}, Webitya, news, ${article.title.toLowerCase().split(" ").slice(0, 3).join(", ")}`
